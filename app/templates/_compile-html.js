...
const compilerHtmlTask = () => {
  ...
  if(kc.template.compiler) {
    // TWIG // Jade
    return <% if ( projectstructure == 'Twig Template' ) { %>gulp.src(kc.src.template + '*.twig')<% } else ( projectstructure == 'Jade Template' ) { %>gulp.src([kc.src.template + '*.jade',kc.src.template + '_*.jade'])<% } %><% if ( projectstructure == 'Twig Template' ) { %>
